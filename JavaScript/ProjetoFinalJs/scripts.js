...
async function convertCurrency() {
    const fromCurrency = document.getElementById('from-currency-select').value.toLowerCase();
    const toCurrency = document.getElementById('to-currency-select').value.toLowerCase();
    ...
    try {
        ...
        if (data && data[fromCurrency] && data[fromCurrency][toCurrency]) {
            ...
            document.getElementById('converted-result').textContent = `${amount} ${fromCurrency.toUpperCase()} = ${convertedAmount.toFixed(2)} ${toCurrency.toUpperCase()}`;
        } else {
